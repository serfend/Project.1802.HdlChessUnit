...
namespace HdlChessUnit
{
	class WinMessager
	{
		...
		public WinMessager(IntPtr target)
		{
			this.Target = target;
		}
		public void SetText( string tmp)
		{
			...
			SendMessage(this.Target, WM_SETTEXT, IntPtr.Zero, new StringBuilder(tmp));
		}
		...
		{
			Target = GetWindow(Target, GW_HWNDPREV);
		}
		...
		public WinMessager GetTarget(IntPtr hwndChildAfter, string lpClassName, string lpWindowName) => GetTarget(this.Target, hwndChildAfter, lpClassName, lpWindowName);
		#endregion
		#region SndMsg
		public void SndMsg(IntPtr hwnd, uint wMsg, IntPtr wParam, StringBuilder lParam)
		{
			new System.Threading.Thread(()=> { SendMessage(hwnd, wMsg, wParam, lParam); }).Start();
		}
		public void SndMsg(uint wMsg, IntPtr wParam , StringBuilder lParam) => SndMsg(this.Target, wMsg, wParam, lParam);
		#endregion
		#region InputStr
		public void InputStr(string Input) => InputStr(Target, Input);
		public void InputStr(IntPtr target, string Input)
		{
		...
		public void InputStr(byte info) => InputStr(Target, info);
		public void InputStr(IntPtr target, byte Input) => KeyEvent(target, WM_CHAR, Input, IntPtr.Zero);
		#endregion
		//第一个窗口
		public const int GW_HWNDFIRST = 0x00;
		//最后一个窗口
		public const int GW_HWNDLAST = 0x01;
		//下一个窗口
		public const int GW_HWNDNEXT = 0x02;
		//上一个窗口
		public const int GW_HWNDPREV = 0x03;
		//主窗口
		public const int GW_OWNER = 0x04;
		//子窗口
		public const int GW_CHILD = 0x05;
		...
		public IntPtr Target { get => target; set => target = value; }
	}
}
